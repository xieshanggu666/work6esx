...
FG.Config = {
  ...
  // 按需铁路运输（FG.Fleet：站点库存上下限 × 空闲列车自动派车 × 在途货量统一扣除）
  FLEET_SCAN_TICKS: 10,      // 派车扫描间隔 tick（0.5 秒一次；任务完成/撤销在每 tick 的收尾中即时处理）
  FLEET_MIN_BATCH: 10,       // 最小派车批量：可供量低于此数不派车（避免零星小批占线）
  FLEET_JOB_TTL: 60 * 20 * 30,// 在途任务台账安全网：超过 30 仿真分钟未收尾的任务视为异常并清理（按 tick）
