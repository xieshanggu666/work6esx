...
class MaterialPool {
  constructor(game) {
    ...
    for (const b of game.map.buildings.values()) {
      ...
      this.chests.push(b);
      // 按需铁路运输的在站预留统一扣除（已派给自动列车的货不参与施工备料争用）
      const fleet = game.fleet;
      for (const s of b.chest) {
        if (s.type && s.count > 0) {
          const hidden = fleet && b.def.railStation ? fleet.reservedAt(b, s.type) : 0;
          const n = Math.max(0, s.count - hidden);
          if (n > 0) this.free.set(s.type, (this.free.get(s.type) || 0) + n);
        }
      }
    }
    this.piles = [];
