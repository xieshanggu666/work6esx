...
FG.Game = class Game {
  constructor() {
    ...
    this.power = new FG.Power(this);       // 电力：电网连通/保供优先级/蓄电池调峰/缺电联动
    this.speed = 1;
    this.paused = false;
    this.tickCount = 0;
    this.playTime = 0;            // 仿真秒
  ...
  serialize() {
    ...
    for (const b of this.map.buildings.values()) {
      blds.push({
        ...
        // 按需铁路运输：站点需求/供货清单（旧档无字段 → 空清单）
        fleetDemands: b.fleetDemands && b.fleetDemands.length ? b.fleetDemands.map(d => ({
          id: d.id, item: d.item, min: d.min, max: d.max, priority: d.priority, enabled: d.enabled !== false,
        })) : undefined,
        fleetSupplies: b.fleetSupplies && b.fleetSupplies.length ? b.fleetSupplies.map(s => ({
          id: s.id, item: s.item, priority: s.priority, enabled: s.enabled !== false,
        })) : undefined,
        // 设备磨损与故障（预测性维护；旧档无字段 → 不磨损/不故障）
        wear: b.wear, wearLimit: b.wearLimit, broken: !!b.broken,
        // 电力：保供优先级（旧档无字段 → 普通）；发电机燃料缓存；蓄电池储能
        powerPriority: b.powerPriority,
        fuel: (typeof b.fuel === 'number' && b.fuel > 0) ? b.fuel : undefined,
        accCharge: (typeof b.accCharge === 'number' && b.accCharge > 0) ? b.accCharge : undefined,
      });
    }
    ...
    return {
      ...
      power: this.power.serialize(),                 // 电力开关（电网图读档惰性重建；燃料/储能随建筑）
  ...
  deserialize(data) {
    ...
    for (const sb of data.buildings) {
      ...
      // 按需铁路运输：站点清单（旧档无字段 → 空清单，行为不变）
      b.fleetDemands = (sb.fleetDemands || []).filter(d => d && FG.Items.byId(d.item)).map(d => ({
        id: d.id || ('D' + Math.random().toString(36).slice(2, 8)),
        item: d.item,
        min: Math.max(1, d.min | 0),
        max: Math.max(2, d.max | 0),
        priority: FG.Config.PRIORITIES[d.priority] ? d.priority : 'normal',
        enabled: d.enabled !== false,
      }));
      b.fleetSupplies = (sb.fleetSupplies || []).filter(s => s && FG.Items.byId(s.item)).map(s => ({
        id: s.id || ('P' + Math.random().toString(36).slice(2, 8)),
        item: s.item,
        priority: FG.Config.PRIORITIES[s.priority] ? s.priority : 'normal',
      ...
      b.broken = !!sb.broken;
      // 电力：保供优先级旧档缺省普通；发电机燃料/蓄电池储能旧档从零开始
      b.powerPriority = FG.Config.PRIORITIES[sb.powerPriority] ? sb.powerPriority : undefined;
      b.fuel = (typeof sb.fuel === 'number' && sb.fuel >= 0) ? sb.fuel : 0;
      b.accCharge = (typeof sb.accCharge === 'number' && sb.accCharge >= 0)
        ? Math.min(sb.accCharge, FG.Config.ACC_CAP_KJ) : 0;
      // 旧存档箱子槽位补齐
      if (b.def.storage) {
    ...
    // 按需运输：规则随车站已读入；自动任务按列车计划中的 fleetJobId 追认，在站预留核账
    this.fleet.deserialize(data.fleet || null);
    ...
    // 电力：开关按存档/科技恢复，电网图不序列化，首个 tick 惰性重建（兼容无 power 字段旧档）
  ...
  tickOnce() {
    this.power.tick();         // 电力：重建电网 → 保供优先级分配 → 蓄电池充放（先定电、后生产）
    this.sim.tick();
    this.construction.tick();   // 施工计划：备料 → 落成
    this.maintenance.tick();    // 维修工单：按优先级预留备件 → 停机检修 → 恢复生产
    this.fleet.preTick();       // 按需运输（列车动作前）：钳制在站装卸上限、扫描派车
    this.railway.tick();        // 铁路：区间占用 → 行驶 → 停站装卸（卸货锁付合同货物）
    this.fleet.postTick();      // 按需运输（列车动作后）：离/到站边沿推进任务、预留核销、收尾撤单
  ...
  placeGhost(x, y) {
    ...
    // 电网节点/用电建筑增删 → 下一 tick 重建电网
    if (FG.Power.isNode(b) || this.power.isConsumer(b)) this.power.markDirty();
  ...
  absorbPile(b) {
    ...
    for (let i = pile.length - 1; i >= 0; i--) {
      ...
      if (b.def.beltTier !== undefined) {
        ...
        // 燃煤发电机：同格遗留的煤炭直接转为燃料缓存（满则留地面）
        if (s.type === 'coal') {
          const took = this.power.refuel(b, 'coal', left);
          left -= took;
        }
      } else if (b.slots) {
  ...
  removeBuilding(b) {
    ...
    // 按需运输：删除站点规则、撤销关联任务（在站预留随 chest 物料落地、在途货保留/退回）
    if (this.fleet && b.def && b.def.railStation) this.fleet.onStationRemoved(b);
    ...
    // 燃煤发电机拆除：燃料缓存按整件煤折算落到地面堆（不足 1 件的零头随拆除消耗）
    if (b.def.powerGen && b.fuel > 0) {
      const coal = Math.floor(b.fuel / FG.Config.POWER_COAL_KJ + 1e-9);
      if (coal > 0) this.map.pileAdd(b.x, b.y, 'coal', coal);
    }
    ...
    // 电网拓扑变化（节点/用电建筑拆除）→ 下一 tick 重建电网
    if (FG.Power.isNode(b) || (this.power && this.power.isConsumer(b))) this.power.markDirty();
