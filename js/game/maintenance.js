...
class SparePool {
  constructor(game) {
    ...
    for (const b of game.map.buildings.values()) {
      ...
      this.chests.push(b);
      // 按需铁路运输的在站预留统一扣除（已派给自动列车的备件不参与维修工单争用）
      const fleet = game.fleet;
      for (const s of b.chest) {
        if (s.type === SPARE && s.count > 0) {
          const hidden = fleet && b.def.railStation ? fleet.reservedAt(b, SPARE) : 0;
          this.free += Math.max(0, s.count - hidden);
        }
      }
    }
    this.piles = [];
