...
FG.Panels = (() => {
  ...
  // ================= 按需铁路运输（车站规则面板，归档 UI 与 React 版功能对齐） =================
  function fleetStationInfo(b) {
    const fleet = FG.game.fleet;
    if (!fleet) return '';
    const solids = FG.Items.list().filter(i => !i.fluid);
    const itemOpts = solids.map(i => `<option value="${i.id}">${i.name}</option>`).join('');
    const prioOpts = [['high', '高'], ['normal', '中'], ['low', '低']]
      .map(([id, name]) => `<option value="${id}">${name}优先</option>`).join('');
    const demands = fleet.demandsOf(b);
    const supplies = fleet.suppliesOf(b);
    let reserve = 0;
    const rm = fleet.reserve.get(fleet.stationKey(b));
    if (rm) for (const v of rm.values()) reserve += v;

    const dRows = demands.map(d => {
      const under = fleet.stockOf(b, d.item) < d.min;
      return `<div class="slot-row" data-fleet-demand="${d.id}">
        <span class="sl-name">${under ? '🔵 ' : ''}${FG.Items.byId(d.item).name}（${fleet.stockOf(b, d.item)}/${d.min}~${d.max}）</span>
        <select data-fd-prio="${d.id}">${['high', 'normal', 'low'].map(p =>
          `<option value="${p}" ${d.priority === p ? 'selected' : ''}>${p === 'high' ? '高' : p === 'normal' ? '中' : '低'}优先</option>`).join('')}</select>
        <button data-fd-toggle="${d.id}">${d.enabled ? '停用' : '启用'}</button>
        <button class="danger" data-fd-del="${d.id}">删</button>
      </div>`;
    }).join('');
    const pRows = supplies.map(s => `<div class="slot-row" data-fleet-supply="${s.id}">
        <span class="sl-name">🟢 ${FG.Items.byId(s.item).name}（库存 ${fleet.stockOf(b, s.item)}）</span>
        <select data-fp-prio="${s.id}">${['high', 'normal', 'low'].map(p =>
          `<option value="${p}" ${s.priority === p ? 'selected' : ''}>${p === 'high' ? '高' : p === 'normal' ? '中' : '低'}优先</option>`).join('')}</select>
        <button data-fp-toggle="${s.id}">${s.enabled ? '停用' : '启用'}</button>
        <button class="danger" data-fp-del="${s.id}">删</button>
      </div>`).join('');

    return `<div class="panel-sec"><h4>按需铁路运输</h4>
      <label class="cfg-row" style="margin:2px 0">
        <input type="checkbox" id="fleet-enabled" ${fleet.enabled ? 'checked' : ''}>
        <span>启用自动派车（在途 ${fleet.jobs.length} 趟 · 本站预留 ${reserve} 件）</span>
      </label>
      <div style="font-size:11px;color:var(--text-dim);margin:4px 0">需求清单（低于下限派车补到上限；合同缺口自动计入）</div>
      ${dRows || '<div style="font-size:11px;color:var(--text-dim)">（无需求规则）</div>'}
      <div class="stop-cfg">
        <select id="fleet-d-item">${itemOpts}</select>
        <input type="number" id="fleet-d-min" class="num-input" value="20" title="下限">
        <input type="number" id="fleet-d-max" class="num-input" value="60" title="上限">
        <select id="fleet-d-prio">${prioOpts}</select>
        <button id="fleet-d-add">＋ 需求</button>
      </div>
      <div style="font-size:11px;color:var(--text-dim);margin:6px 0 2px">供货清单（本站可供货物品）</div>
      ${pRows || '<div style="font-size:11px;color:var(--text-dim)">（无供货规则）</div>'}
      <div class="stop-cfg">
        <select id="fleet-p-item">${itemOpts}</select>
        <select id="fleet-p-prio">${prioOpts}</select>
        <button id="fleet-p-add">＋ 供货</button>
      </div>
    </div>`;
  }

  /** 绑定车站按需运输面板事件 */
  function bindFleetStation(b) {
    const fleet = FG.game.fleet;
    if (!fleet) return;
    const en = document.getElementById('fleet-enabled');
    if (en) en.onchange = () => fleet.setEnabled(en.checked);
    const dAdd = document.getElementById('fleet-d-add');
    if (dAdd) dAdd.onclick = () => {
      fleet.addDemand(b, document.getElementById('fleet-d-item').value,
        parseInt(document.getElementById('fleet-d-min').value, 10) || 20,
        parseInt(document.getElementById('fleet-d-max').value, 10) || 60,
        document.getElementById('fleet-d-prio').value);
      renderInfo();
    };
    const pAdd = document.getElementById('fleet-p-add');
    if (pAdd) pAdd.onclick = () => {
      fleet.addSupply(b, document.getElementById('fleet-p-item').value,
        document.getElementById('fleet-p-prio').value);
      renderInfo();
    };
    document.querySelectorAll('[data-fd-del]').forEach(el => el.onclick = () => {
      fleet.removeDemand(b, el.dataset.fdDel); renderInfo();
    });
    document.querySelectorAll('[data-fp-del]').forEach(el => el.onclick = () => {
      fleet.removeSupply(b, el.dataset.fpDel); renderInfo();
    });
    document.querySelectorAll('[data-fd-toggle]').forEach(el => el.onclick = () => {
      const d = fleet.demandsOf(b).find(x => x.id === el.dataset.fdToggle);
      if (d) fleet.updateDemand(b, d.id, { enabled: !d.enabled });
    });
    document.querySelectorAll('[data-fp-toggle]').forEach(el => el.onclick = () => {
      const s = fleet.suppliesOf(b).find(x => x.id === el.dataset.fpToggle);
      if (s) fleet.updateSupply(b, s.id, { enabled: !s.enabled });
    });
    document.querySelectorAll('[data-fd-prio]').forEach(el => el.onchange = () =>
      fleet.updateDemand(b, el.closest('[data-fleet-demand]').dataset.fleetDemand, { priority: el.value }));
    document.querySelectorAll('[data-fp-prio]').forEach(el => el.onchange = () =>
      fleet.updateSupply(b, el.closest('[data-fleet-supply]').dataset.fleetSupply, { priority: el.value }));
  }
  ...
  function bindActions() {
    ...
    if (sel && sel.def && sel.def.railStation) bindFleetStation(sel);
    bindContractActions();
